...
| Career Horizontal Feature Reveal | https://21st.dev/@hyperiux/components/horizontal-feature-reveal | `src/components/resume/resume-career-reveal.tsx` | Existing scoped GSAP/ScrollTrigger bridge; no SplitText dependency | Adapts the vertical-to-horizontal editorial progression to two typed employer entries. It removes the source images, parallax, SplitText masks, fixed card-gap assumptions, smooth-scroll ownership, and DOM-moving pin; the initial/server, reduced-motion, coarse-pointer, narrow, short, zoom, and hydration-failure result is the existing `ResumeTimeline`, with exactly one accessible chronology at a time. The enhanced track remains scoped and non-pinning so breakpoint fallback swaps cannot move React-owned nodes. | Re-verified 2026-09-08: public code/usage page, author Hyperiux Vault, GSAP dependency, and MIT License declared; immutable revision not exposed. |
| Credential Stacking Cards | https://21st.dev/@danielpetho/components/stacking-cards | `src/components/resume/resume-credential-stack.tsx` | Existing Motion only; no image dependency | Adapt sticky/scale presentation to the three existing education entries with a semantic ordered list, readable dwell intervals, and natural-flow fallbacks; no new media or claims | Public source inspected 2026-09-08; author Daniel Petho; attribution retained for the implemented adaptation |
...
| Google Gemini Effect | https://ui.aceternity.com/components/google-gemini-effect | `src/components/resume/resume-convergence.tsx` | Existing Motion; no WebGL | Adapt the MotionValue-driven SVG path treatment as five restrained token-colored decorative paths leading from Languages to the existing PDF, contact, and Return to Work actions; remove demo copy | Public component and props inspected 2026-09-08; Aceternity UI; attribution retained for the implemented adaptation |
