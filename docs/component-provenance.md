...
| --- | --- | --- | --- | --- | --- |
| Identity Scroll Choreography | https://21st.dev/@componentry/components/scroll-choreography | `src/components/resume/resume-identity-scene.tsx` | Existing Motion runtime only; no source package | Directly copied the four-image choreography structure and staged movement into the Resume scene, using existing portfolio media and retaining the semantic Resume facts/actions. | Public source inspected 2026-09-08 from Componentry repository/registry; author Harsh Jadhav / componentry; repository MIT; source implementation copied into the local component and wired to the shared Resume progress. |
| Capability Radial Orbital Timeline | https://21st.dev/@jatin-yadav05/components/radial-orbital-timeline | `src/components/resume/resume-capability-orbit.tsx` | Existing Motion runtime only; no `lucide-react` dependency | Directly reproduced the public preview implementation's rotating radial nodes, glow, selected node card, and center treatment while feeding it the six existing Resume skill groups and preserving a semantic list fallback. | Public source/preview inspected 2026-09-08; author Jatin Yadav; MIT License declared; implementation copied into the local component from the public preview bundle. |
