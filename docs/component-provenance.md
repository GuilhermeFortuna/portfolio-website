...
| Career Horizontal Feature Reveal | https://21st.dev/@hyperiux/components/horizontal-feature-reveal | `src/components/resume/resume-career-reveal.tsx` | Existing scoped GSAP/ScrollTrigger bridge; no SplitText dependency | Adapts the vertical-to-horizontal editorial progression to two typed employer entries. It removes the source images, parallax, SplitText masks, fixed card-gap assumptions, and smooth-scroll ownership; the initial/server, reduced-motion, coarse-pointer, narrow, short, zoom, and hydration-failure result is the existing `ResumeTimeline`, with exactly one accessible chronology at a time. | Re-verified 2026-09-08: public code/usage page, author Hyperiux Vault, GSAP dependency, and MIT License declared; immutable revision not exposed. |
