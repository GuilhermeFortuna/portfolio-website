...
| Credential Stacking Cards (planned) | https://21st.dev/@danielpetho/components/stacking-cards | WO-048 Resume credential scene | Existing Motion only; no image dependency | Adapt sticky/scale presentation to the three existing education entries; Languages remains a calm semantic list; no new media or claims | Public code/usage inspected 2026-09-08; author Daniel Petho (example credits Khoa Phan); license and immutable revision not exposed; verify before implementation |
