...
**Batch indexes:** [`README.md`](README.md) (Batch 01), [`BATCH-02-README.md`](BATCH-02-README.md) (Batch 02)
...
Batch 02 is planned only. WO-012 remains `IMPLEMENTING`, so no Batch 02 order may be dispatched yet. Its planned scope assumes Batch 01's production dependencies and page structure are retained.
...
| [WO-012](WO-012-integration-accessibility-performance.md) | `REVIEW` | WO-005–WO-011 | GPT-5.6 Sol | `development` (uncommitted) | Integration complete. Evidence: Shape Blur desktop-only mount; Logo Loop token cleanup; header `min-w-11`; body `min-width: 320px`; GSAP dynamically imported only when motion is allowed; favicon added; provenance + README updated. Automated: `npm run lint`, `npm run typecheck`, `npm run build` pass. Manual matrices recorded under `.superpowers/sdd/` (viewport, a11y, motion, runtime). Lighthouse desktop Accessibility 96 / Best Practices 96 / Performance 98. Awaiting independent acceptance review. |
| [WO-013](WO-013-test-foundation.md) | `BLOCKED` | WO-012 | — | — | Wait for WO-012 independent review and `DONE`. |
| [WO-014](WO-014-content-utility-tests.md) | `BLOCKED` | WO-013 | — | — | Test foundation must be reviewed first. |
| [WO-015](WO-015-static-component-tests.md) | `BLOCKED` | WO-013 | — | — | Test foundation must be reviewed first. |
| [WO-016](WO-016-interactive-hook-tests.md) | `BLOCKED` | WO-013–WO-015 | — | — | Shared test conventions and preceding test coverage must be reviewed first. |
| [WO-017](WO-017-test-baseline-review.md) | `BLOCKED` | WO-014–WO-016 | — | — | All Batch 02 implementation orders must be reviewed first. |
