...
| [WO-008](WO-008-project-showcase-structure.md) | `REVIEW` | WO-002, WO-003 | Auto (WO-008 project showcase) | `feat/wo-008-project-showcase` | Client `ProjectShowcase` with desktop selectors + sticky abstract stage, mobile articles, `[data-sparkles-slot]` and `[data-shape-blur-slot]`. `npm run lint`, `npm run typecheck`, and `npm run build` passed. Manual checks still needed (click/focus/fine-hover, coarse pointer, &lt;1024 articles, JS-off visibility). |
